...
# NoteL This script manually overwrites the postcode of one coral store which was listed incorrectly on the government register.
...
# For consistency and so that we can join the
postcode_centeroids = postcode_centeroids %>%
                  mutate(pcd = gsub(" ", "", pcd))


# Left join the locations with postcode
...
# Remove the spaces in the postcodes of the stores
store_locations = store_locations %>%
  mutate(pcd = gsub(" ", "", pcd))
